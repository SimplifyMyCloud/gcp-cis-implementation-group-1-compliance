...
Everything below assumes the shell is already set up — signed in, on your branch, impersonating the auditor, with `config/audit.env` in place. That is [auditor setup](../cis-ig1-auditor-setup.md), or `audit-on` once its Cloud Shell setup is done.
