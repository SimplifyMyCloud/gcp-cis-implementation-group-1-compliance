...
Found by running the audit end to end against a live organization. Every entry has **evidence
...
Both passes finished `RUN STATUS: OK` with 0 DENIED and 0 ERROR once the APIs
below were enabled, which is what makes this list complete rather than a guess.
...
Verified directly: `gcloud container clusters list --project=AUDITED_PROJECT
--billing-project=AUDIT_HOST_PROJECT` still failed with *Kubernetes Engine API has not been used in
project AUDITED_PROJECT*. Cloud SQL Admin is the counter-intuitive one: its calls bill to the
...
| 14 | `securitycenter.googleapis.com` | V115 (V186 via cross-reference) | ListNotificationConfigs | **error** runs 3–6: `API DISABLED IN AUDIT HOST PROJECT: securitycenter.googleapis.com`; enabled → OK in run 8 | 2026-09-13 |
| 15 | `serviceusage.googleapis.com` | V24 V45 V122 | ListServices | metrics — billed to host | 2026-09-14 |
| 16 | `spanner.googleapis.com` | V157 | ListInstances | **error** run 7: `API [spanner.googleapis.com] not enabled on project [AUDIT_HOST_PROJECT]`; enabled → OK in run 8 | 2026-09-13 |
...
These were observed billed to the audited project. When one is disabled in an audited
