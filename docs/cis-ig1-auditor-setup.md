...
|---|---|---|
| Organization ID | `933250405420` | `gcloud organizations list` |
...
`whoami` in Cloud Shell is your account name without the domain, so `chris@simplifymy.cloud` gets `chris/cis-ig1-audit`. Type the name yourself if you work locally under a different user.
...
### 7. Build the output directory and config

Everything the run produces goes under `audit-state/`, which is committed on your branch:
...
| Path | Holds |
|---|---|
| `audit-state/audit.env` | The config for this engagement |
...
Write the config template only if one does not already exist. `-init-config` overwrites an existing file without asking, which would lose the values you set on an earlier day:

```bash
[ -f ./audit-state/audit.env ] || go run audit-run.go -init-config ./audit-state/audit.env
```

Open it and set `APPROVED_REGISTRIES`:

```bash
cloudshell edit ./audit-state/audit.env
```

Locally, use `vi ./audit-state/audit.env` or your editor.
...
```bash
./run-audit.sh --config ./audit-state/audit.env --out ./audit-state/runs --all
```
